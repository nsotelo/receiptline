...
(function() {
  ...
  function transform(doc, printer) {
    ...
    const ptr = {
      cpl: printer.cpl || 48,
      encoding: /^(cp(437|85[28]|86[0356]|1252|93[26]|949|950)|shiftjis|gb18030|ksc5601|big5)$/.test(
        printer.encoding
      )
        ? printer.encoding
        : "cp437",
    ...
    const res = doc
      .normalize()
      .split(/\n|\r\n|\r/)
      .map(line => createLine(parseLine(line), ptr));
        ...
        .reduce((obj, member) => {
          ...
          if (
            ...
            member.replace(
              /^[\t ]*([A-Za-z_]\w*)[\t ]*:[\t ]*([^\t ].*?)[\t ]*$/,
              (match, key, value) =>
                (obj[key.replace(/^[abciotwx_]$/, m => abbr[m])] = parseEscape(
                  value.replace(/\\n/g, "\n")
                ))
            ) === member
          ) {
      ...
      if (array.length === 1) {
        ...
        if ("option" in result.property) {
          ...
          state.option = {
            ...
            height: Number(
              option.find(
                c => /^\d+$/.test(c) && Number(c) >= 24 && Number(c) <= 240
              ) || "72"
            ),
  ...
  const _svg = {
    ...
    hr: function(width) {
      ...
      const path = `<path d="M0,${w}h${w *
        width}" fill="none" stroke="#000" stroke-width="2"/>`;
    ...
    vrhr: function(widths1, widths2, dl, dr) {
      ...
      const path1 =
        ...
        `" fill="none" stroke="#000" stroke-width="2"/>`;
      this.svgContent += `<g transform="translate(${(this.lineMargin +
        Math.max(-dl, 0)) *
        w},${this.svgHeight})">${path1}</g>`;
      const path2 =
        `<path d="` +
        widths2
          .reduce(
    ...
    // cut paper:
    cut: function() {
      const path = `<path d="M12,12.5l-7.5,-3a2,2,0,1,1,.5,0M12,11.5l-7.5,3a2,2,0,1,0,.5,0" fill="none" stroke="#000" stroke-width="1"/><path d="M12,12l10,-4q-1,-1,-2.5,-1l-10,4v2l10,4q1.5,0,2.5,-1z" fill="#000"/><path d="M24,12h${this
        .svgWidth -
        24}" fill="none" stroke="#000" stroke-width="2" stroke-dasharray="2"/>`;
    ...
    lf: function() {
      ...
      if (this.textElement.length > 0) {
        this.svgContent += `<g transform="translate(${this.lineMargin *
          this.charWidth},${this.svgHeight + h})">${this.textElement}</g>`;
      }
  ...
  const _sbcs = {
    ...
    vrtable: {
      ...
      "\xbf": { " ": "\xbf", À: "\xc5", Á: "\xc5", Ä: "\xc2", Ù: "\xb4" },
      Â: { " ": "\xc2", À: "\xc5", Á: "\xc5", Ä: "\xc2", Ù: "\xc5" },
      Ä: { " ": "\xc4", À: "\xc1", Á: "\xc1", Ä: "\xc4", Ù: "\xc1" },
      Ú: { " ": "\xda", À: "\xc3", Á: "\xc5", Ä: "\xc2", Ù: "\xc5" }
    }
  };
  ...
  const commands = {
    ...
    emustarlinembcs2: Object.assign(
      Object.create(_base),
      _star,
      _line,
      _emu,
